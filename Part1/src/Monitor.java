...
public class Monitor implements Runnable {
		...
		for (i = 0 ; i < data.NumOfWireless() ; i++) {  
			...
			if (j == numberOfInterfaces) {
				...
				if ( data.NumOfWireless() == 0 ) {
					if ( data.sys1 == false ) { System.out.println("Attention, there are no any wireless interfaces so on..."); }
	...
	private void insert_Interface() {
		if ( data.sys1 == false ) { System.out.println("inserted"); }
	}
	
	
	private void remove_Interface() {
		if ( data.sys1 == false ) { System.out.println("removed"); }
		if ( data.sys1 == false ) { System.out.println("Stoping monitor for interface : " + data.getWired(i).get_InterfaceName()); }
	}
	...
	private void forceFindNewScanner() {
		...
		if ( data.sys1 == false ) { System.out.println("found"); }
	}	
	...
	private void printAttention() {
		...
		if ( data.sys1 == false ) { System.out.println("attention"); }
	}
	...
	public void run() {	
		...
		while ("the truth about forever is that it is happening right now" != null) {
			...
			if ( data.sys1 == false ) { System.out.println("-------------------------------------"); }
